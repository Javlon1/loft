...
const Interest = () => {
    ...
    const [interestData] = React.useState([{ id: 1, nav_en: 'Might interest', nav_ru: 'Может заинтересовать', nav_uz: 'Qiziqtirishi mumkin' }]);
    const [interestTitle] = React.useState([{ id: 1, nav_en: 'Buy', nav_ru: 'Купить', nav_uz: 'Sotib olish' }]);
        ...
        <section className={styles.interest}>
            <MyContainer>
                <div className={styles.interest__item}>
                    <div className={styles.interest__item__header}>
                        <div data-aos="fade-up" className={styles.interest__item__header__title}>
                            {
                        ...
                        <div className={styles.interest__item__header__btns}>
                    ...
                    >
                        {
                            data?.map((item) => (
                                <SwiperSlide key={item.id}>
                                    <div className={styles.interest__item__cart}>
                                        <div className={styles.interest__item__cart__item}>
                                            <Link
                                                className={styles.interest__item__cart__item__img}
                                                href={'/'}
                                            >
                                                <Image
                                                    src={item.image}
                                            ...
                                            <p>{item.price}</p>
                                            {
                                                interestTitle?.map((item) => (
                                                    <Link
                                                        className={styles.interest__item__cart__item__link}
                                                        href={'/'}
                                                        key={item.id}
                                                    >
                                                        {item[`nav_${lan}`]}
